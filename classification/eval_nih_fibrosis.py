...
from classification.util.datasets_pneum import Fibrosis_Dataset
...
def get_args_parser():
    ...
    parser.add_argument('--pretrained_pth', default='.../nih_fibrosis_trainedpt/SLGMS_nih_fibrosis.pth', type=str,
                        help='Please specify path to the ImageNet training data.')
    parser.add_argument('--output_dir', default=".../nih_fibrosis/", type=str, help='Path to save logs and checkpoints.')
...
def eval_slgms(config, args):
    ...
    print(f"Data loaded: there are {len(test_loader)} images.")
    ...
    print(msg)
    validate_network(test_loader, model)
...
def validate_network(val_loader, model):
    ...
    all_outputs = []
    all_targets = []
    for it, [inp, target, imgname] in enumerate(val_loader):
        # move to gpu
        images = inp.cuda(non_blocking=True)
        # forward
        with torch.no_grad():
            cls_output = model(images)
            # 收集输出和目标
            all_outputs.append(cls_output)
            all_targets.append(target)
        score = F.softmax(cls_output.float().cpu().detach(), dim=-1)
        global_score=score
        labelname = imgname[0].split('_')[0]
        result = {}
        result["Image Index"] = imgname[0]
        result["Finding Labels"] = labelname
        pred_global_labels = ""
        aa = torch.argmax(global_score)
        if aa == 0:
            pred_global_labels = pred_global_labels + 'Sick'
        else:
            pred_global_labels = pred_global_labels + 'Health'
        result["Pred Labels"] = pred_global_labels
        result["Pred logits"] = '%.4f' % global_score[0,aa].cpu().numpy()
        data_results.append(result)
    df_data_results = pd.DataFrame(data_results)
    df_data_results.to_csv(args.output_dir + '/test_data_result_scores.csv', index=False)
    # 合并所有输出和目标
    ...
    df.to_csv(args.output_dir + '/test_data_result.csv', index=False)
    print(avg)
