...
from classification.util.datasets_pneum import Fibrosis_Dataset, Fibrosis_Mixed_Mask_Dataset_DINO
import pandas as pd
...
def get_args_parser():
    ...
    parser.add_argument("--base_lr", default=5e-5, type=float, help="""Learning rate at the end of
    ...
    # Misc
    parser.add_argument('--data_path', default='.../NIH_fibrosis_dataset/', type=str,
                        help='Please specify path to the ImageNet training data.')
    parser.add_argument('--output_dir', default='.../NIHFibrosis_448/', type=str,
...
def train_dino(config, args):
    ...
    dataset_train = Fibrosis_Mixed_Mask_Dataset_DINO(args.data_path + '/vis_448_process/seg_rec_image',
                                                 args.data_path + '/vis_448_process/seg_rec_mask',
    ...
    dataset_val = Fibrosis_Dataset(args.data_path + '/vis_448_process/seg_rec_image',
                                   txtpath=args.data_path + '/val.txt',
                                 data_transform=val_transform)
    dataset_test = Fibrosis_Dataset(args.data_path + '/vis_448_process/seg_rec_image',
                                   txtpath=args.data_path + '/test.txt',
                                 data_transform=val_transform)
    ...
    print("Training of the supervised linear classifier on frozen features completed.\n"
          "Top-1 test avg: {avg:.1f}".format(avg=best_avg))
    ...
    test_acc1,test_auc,test_Sensitivity,test_Specificity, test_avg = validate_network(test_loader, teacher)
    return test_acc1,test_auc,test_Sensitivity,test_Specificity, test_avg
...
def train_one_epoch(student, teacher, teacher_without_ddp, dino_loss, data_loader,
    ...
    for it, (images, targets,_) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            # tea_cls_output, teacher_output, _ = teacher(images[:2], masks, targets, last_fea_return=True, last_attn_return=False)  # only the 2 global views pass through the teacher
...
class DINOLoss(nn.Module):
    def __init__(self, out_dim, ncrops, warmup_teacher_temp, teacher_temp,
        ...
        imgsize=224
        self.patch_size=16
        self.featmaphw = imgsize // self.patch_size
...
if __name__ == '__main__':
    ...
    config.freeze()
    test_acc, test_auc, test_Sensitivity, test_Specificity, test_avg = train_dino(config, args)