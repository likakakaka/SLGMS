...
class Compose:
    ...
    def __call__(self, img, mask, imgname=None):
        for t in self.transforms:
            img, mask = t(img, mask, imgname)
        return img, mask
...
class ToTensor:
    ...
    def __call__(self, pic1, pic2, imgname=None):
        """
        Args:
...
class Normalize(torch.nn.Module):
    ...
    def forward(self, tensor1: Tensor,  tensor2: Tensor, imgname=None):
        """
        Args:
        ...
        """
        return F.normalize(tensor1, self.mean, self.std, self.inplace), tensor2
...
class RandomCenterResizedCropHW(torch.nn.Module):
    ...
    def get_params(img: Tensor, scale: List[float], ratio: List[float]) -> Tuple[int, int, int, int]:
        ...
        for _ in range(10):
            ...
            w = torch.randint(int(width * ratio[0]), int(width * ratio[1]), size=(1,)).item()
            h = torch.randint(int(height * ratio[0]), int(height * ratio[1]), size=(1,)).item()
...
class RandomEqualize2(torch.nn.Module):
